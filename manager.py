...
class Manager:
    ...
    def connect(self):
        s = cisco_ncclient.transport.Session(ip=self.ip, username=self.username, password=self.password)
        s.set_channel()
        s.hello_netconf()
        self.__session = s
    ...
    def edit_config(self, filename):
        ...
        rpc = cisco_ncclient.operations.EditConfigRPC()
    ...
    def get_config(self):
        temp_chan = cisco_ncclient.transport.Session(ip=self.ip, username=self.username, password=self.password)
